...
length(FinalVariableList) # final variable list is now 154
```
...
#heatmap(FinalVariableList.heat.log, Rowv=NA, Colv=NA, col=colorRampPalette(c("white", "red"))(n=100), scale='none')
FinalVariableList.heat.log[is.na(FinalVariableList.heat.log)] <- 0
FinalVariableList.heat.log[lower.tri(FinalVariableList.heat.log)] <- NA
heatmap(FinalVariableList.heat.log, col=colorRampPalette(c("white", "red"))(n=100), scale='none', Rowv=NA, Colv=NA)
```
...
# age above analysis - OLD
 dsn.age.above <- subset(dsn.all, RIDAGEYR >= 65)
surv.age.above.raw <- xwas(design=dsn.age.above, adjfor=adjustfor[!(adjustfor %in% c("RIDAGEYR", "RIDAGEYR2"))])
surv.age.above.coef <- lapply(surv.age.above.raw, function(x) { coef(summary(x)) })
surv.age.above.r2 <- lapply(surv.age.above.raw, function(x) { weightedr2(x) }) # list of df
surv.age.above <- xwas2df(surv.age.above.coef)
surv.age.above <- annotate(surv.age.above)
surv.age.above.sig <- subset(surv.age.above, q < 0.05)
surv.age.above.sig <- surv.age.above.sig[order(surv.age.above.sig$beta), ]

# age below analysis - YOUNG
 dsn.age.below <- subset(dsn.all, RIDAGEYR < 65)
surv.age.below.raw <- xwas(design=dsn.age.below, adjfor=adjustfor[!(adjustfor %in% c("RIDAGEYR", "RIDAGEYR2"))])
surv.age.below.coef <- lapply(surv.age.below.raw, function(x) { coef(summary(x)) })
surv.age.below.r2 <- lapply(surv.age.below.raw, function(x) { weightedr2(x) }) # list of df
surv.age.below <- xwas2df(surv.age.below.coef)
surv.age.below <- annotate(surv.age.below)
surv.age.below.sig <- subset(surv.age.below, q < 0.05)
surv.age.below.sig <- surv.age.below.sig[order(surv.age.below.sig$beta), ]

# race analysis - LATIN
 dsn.race.latin <- subset(dsn.all, RIDRETH1 == 1) # only mexican-american
#dsn.race.latin <- subset(dsn.all, RIDRETH1 == 1 | RIDRETH1 == 2) # mexican-american and other latin
surv.race.latin.raw <- xwas(design=dsn.race.latin, adjfor=adjustfor[adjustfor != "RIDRETH1"])
surv.race.latin.coef <- lapply(surv.race.latin.raw, function(x) { coef(summary(x)) })
surv.race.latin.r2 <- lapply(surv.race.latin.raw, function(x) { weightedr2(x) }) # list of df
surv.race.latin <- xwas2df(surv.race.latin.coef)
surv.race.latin <- annotate(surv.race.latin)
surv.race.latin.sig <- subset(surv.race.latin, q < 0.05)
surv.race.latin.sig <- surv.race.latin.sig[order(surv.race.latin.sig$beta), ]

# race analysis - WHITE
 dsn.race.white <- subset(dsn.all, RIDRETH1 == 3)
surv.race.white.raw <- xwas(design=dsn.race.white, adjfor=adjustfor[adjustfor != "RIDRETH1"])
surv.race.white.coef <- lapply(surv.race.white.raw, function(x) { coef(summary(x)) })
surv.race.white.r2 <- lapply(surv.race.white.raw, function(x) { weightedr2(x) }) # list of df
surv.race.white <- xwas2df(surv.race.white.coef)
surv.race.white <- annotate(surv.race.white)
surv.race.white.sig <- subset(surv.race.white, q < 0.05)
surv.race.white.sig <- surv.race.white.sig[order(surv.race.white.sig$beta), ]

# race analysis - BLACK
 dsn.race.black <- subset(dsn.all, RIDRETH1 == 4)
surv.race.black.raw <- xwas(design=dsn.race.black, adjfor=adjustfor[adjustfor != "RIDRETH1"])
surv.race.black.coef <- lapply(surv.race.black.raw, function(x) { coef(summary(x)) })
surv.race.black.r2 <- lapply(surv.race.black.raw, function(x) { weightedr2(x) }) # list of df
surv.race.black <- xwas2df(surv.race.black.coef)
surv.race.black <- annotate(surv.race.black)
surv.race.black.sig <- subset(surv.race.black, q < 0.05)
surv.race.black.sig <- surv.race.black.sig[order(surv.race.black.sig$beta), ]
...
```{r eval=FALSE}
metaforest <- function(x, group="N/A", base="") {
  for (i in names(x)) {
    png(file=paste0("~/src/phenome-mortality/forest/", base, i, ".png"))
    forest(x[[i]], main=paste0("\n\n\n\n\n", group, "\n\n", VarDescription[VarDescription$var == i, ]$var_desc[1]))
    dev.off()
  }
}
metaforest(surv.all.meta.raw, "All-cause", "all")
```
...
com.list.age.above   <- rownames(surv.age.above.sig)
com.list.age.below   <- rownames(surv.age.below.sig)
com.list.race.latin  <- rownames(surv.race.latin.sig)
com.list.race.white  <- rownames(surv.race.white.sig)
com.list.race.black  <- rownames(surv.race.black.sig)
...
com.df.age.above   <- cbind(com.df.age.above,   com.list.age.above   %in% rownames(surv.age.above.sig))
com.df.age.below   <- cbind(com.df.age.below,   com.list.age.below   %in% rownames(surv.age.below.sig))
com.df.race.latin   <- cbind(com.df.race.latin,   com.list.race.latin   %in% rownames(surv.race.latin.sig))
com.df.race.white   <- cbind(com.df.race.white,   com.list.race.white   %in% rownames(surv.race.white.sig))
com.df.race.black   <- cbind(com.df.race.black,   com.list.race.black   %in% rownames(surv.race.black.sig))
...
colnames(com.df.age.above)   <- c("Co-morbidity", "1999-2010", AvailableSeries)
colnames(com.df.age.below)   <- c("Co-morbidity", "1999-2010", AvailableSeries)
colnames(com.df.race.latin)   <- c("Co-morbidity", "1999-2010", AvailableSeries)
colnames(com.df.race.white)   <- c("Co-morbidity", "1999-2010", AvailableSeries)
colnames(com.df.race.black)   <- c("Co-morbidity", "1999-2010", AvailableSeries)
...
upset(com.df.f,   sets=c("1999-2010", rev(AvailableSeries)), keep.order=T, order.by="freq", sets.x.label="Significant Comorbidities \n (q < 0.05)", mainbar.y.label="Unique Comorbidities \n (Female only)",
    ...
    queries=list(
                  ...
                  list(query=intersects, params=list("1999-2010", "1999-2000", "2001-2002", "2003-2004", "2005-2006"), color="red", active=T),
                  list(query=intersects, params=list("1999-2010", "1999-2000", "2001-2002", "2003-2004", "2005-2006", "2007-2008"), color="red", active=T),
                  list(query=intersects, params=list("1999-2010", "1999-2000", "2001-2002", "2005-2006"), color="red", active=T),
...
upset(com.df.age.above, sets=c("1999-2010", rev(AvailableSeries)), keep.order=T, order.by="freq", sets.x.label="Significant Comorbidities \n (q < 0.05)", mainbar.y.label="Unique Comorbidities \n (Age 62 and Older)",
      queries=list(
                  list(query=intersects, params=list("1999-2010", "1999-2000", "2001-2002", "2003-2004", "2005-2006", "2007-2008", "2009-2010"), color="red", active=T),
                  list(query=intersects, params=list("1999-2010", "1999-2000", "2001-2002", "2003-2004", "2007-2008", "2009-2010"), color="red", active=T),
                  list(query=intersects, params=list("1999-2010", "1999-2000", "2001-2002", "2003-2004"), color="red", active=T),
                  ...
                  list(query=intersects, params=list("1999-2010", "2001-2002", "2003-2004", "2005-2006", "2007-2008", "2009-2010"), color="red", active=T),
                  list(query=intersects, params=list("1999-2010", "1999-2000", "2001-2002", "2003-2004", "2009-2010"), color="red", active=T),
                  list(query=intersects, params=list("1999-2010", "2001-2002", "2005-2006", "2009-2010"), color="red", active=T),
                  list(query=intersects, params=list("1999-2010", "1999-2000", "2001-2002", "2007-2008", "2009-2010"), color="red", active=T),
                  list(query=intersects, params=list("1999-2010", "1999-2000", "2003-2004", "2007-2008", "2009-2010"), color="red", active=T),
                  list(query=intersects, params=list("1999-2010", "2005-2006", "2007-2008", "2009-2010"), color="red", active=T)
                )
)

upset(com.df.age.below,   sets=c("1999-2010", rev(AvailableSeries)), keep.order=T, order.by="freq", sets.x.label="Significant Comorbidities \n (q < 0.05)", mainbar.y.label="Unique Comorbidities \n (Age Younger Than 62)",
    queries=list(
                  list(query=intersects, params=list("1999-2010", "1999-2000", "2001-2002", "2003-2004", "2005-2006"), color="red", active=T),
                  list(query=intersects, params=list("1999-2010", "1999-2000", "2001-2002", "2003-2004", "2007-2008"), color="red", active=T),
                  list(query=intersects, params=list("1999-2010", "1999-2000", "2001-2002", "2003-2004"), color="red", active=T),
                  ...
                  list(query=intersects, params=list("1999-2010", "1999-2000", "2001-2002", "2003-2004", "2007-2008", "2009-2010"), color="red", active=T),
                  list(query=intersects, params=list("1999-2010", "2001-2002", "2003-2004", "2005-2006"), color="red", active=T),
                  list(query=intersects, params=list("1999-2010", "1999-2000", "2001-2002", "2007-2008"), color="red", active=T),
                  list(query=intersects, params=list("1999-2010", "2001-2002", "2003-2004", "2005-2006", "2007-2008"), color="red", active=T),
                  list(query=intersects, params=list("1999-2010", "1999-2000", "2001-2002", "2003-2004", "2005-2006", "2007-2008"), color="red", active=T),
                  list(query=intersects, params=list("1999-2010", "1999-2000", "2001-2002", "2003-2004", "2009-2010"), color="red", active=T),
                  list(query=intersects, params=list("1999-2010", "2003-2004", "2007-2008", "2009-2010"), color="red", active=T),
...
com.list.age.above <- intersect(com.df.age.above[com.df.age.above$`1999-2010` == 1,]$`Co-morbidity`,
                        com.df.age.above$`Co-morbidity`[apply(com.df.age.above[, 3:ncol(com.df.age.above)], 1, sum) > 2])

com.list.age.below <- intersect(com.df.age.below[com.df.age.below$`1999-2010` == 1,]$`Co-morbidity`,
                        com.df.age.below$`Co-morbidity`[apply(com.df.age.below[, 3:ncol(com.df.age.below)], 1, sum) > 2])

com.list.race.latin <- intersect(com.df.race.latin[com.df.race.latin$`1999-2010` == 1,]$`Co-morbidity`,
                        com.df.race.latin$`Co-morbidity`[apply(com.df.race.latin[, 3:ncol(com.df.race.latin)], 1, sum) > 2])

com.list.race.white <- intersect(com.df.race.white[com.df.race.white$`1999-2010` == 1,]$`Co-morbidity`,
                        com.df.race.white$`Co-morbidity`[apply(com.df.race.white[, 3:ncol(com.df.race.white)], 1, sum) > 2])

com.list.race.black <- intersect(com.df.race.black[com.df.race.black$`1999-2010` == 1,]$`Co-morbidity`,
                        com.df.race.black$`Co-morbidity`[apply(com.df.race.black[, 3:ncol(com.df.race.black)], 1, sum) > 2])
...
com.age.above <- subset(surv.age.above.sig[com.list.age.above, ], abs(beta) > 0.25)
#com.age.above <- surv.age.above.sig[com.list.age.above, ]
com.age.above <- merge(com.age.above, unlist(surv.age.above.r2), by="row.names", all.x=TRUE)
row.names(com.age.above) <- com.age.above$Row.names
com.age.above$Row.names <- NULL
colnames(com.age.above)[ncol(com.age.above)] <- "r2"
com.age.above$p2 <- pnorm(abs(com.age.above$beta/com.age.above$se), lower.tail=F)*2
com.age.above <- com.age.above[order(com.age.above$beta),]

com.age.below <- subset(surv.age.below.sig[com.list.age.below, ], abs(beta) > 0.25)
#com.age.below <- surv.age.below.sig[com.list.age.below, ]
com.age.below <- merge(com.age.below, unlist(surv.age.below.r2), by="row.names", all.x=TRUE)
row.names(com.age.below) <- com.age.below$Row.names
com.age.below$Row.names <- NULL
colnames(com.age.below)[ncol(com.age.below)] <- "r2"
com.age.below$p2 <- pnorm(abs(com.age.below$beta/com.age.below$se), lower.tail=F)*2
com.age.below <- com.age.below[order(com.age.below$beta),]

com.race.latin <- subset(surv.race.latin.sig[com.list.race.latin, ], abs(beta) > 0.25)
#com.race.latin <- surv.race.latin.sig[com.list.race.latin, ]
com.race.latin <- merge(com.race.latin, unlist(surv.race.latin.r2), by="row.names", all.x=TRUE)
row.names(com.race.latin) <- com.race.latin$Row.names
com.race.latin$Row.names <- NULL
colnames(com.race.latin)[ncol(com.race.latin)] <- "r2"
com.race.latin$p2 <- pnorm(abs(com.race.latin$beta/com.race.latin$se), lower.tail=F)*2
com.race.latin <- com.race.latin[order(com.race.latin$beta),]

com.race.white <- subset(surv.race.white.sig[com.list.race.white, ], abs(beta) > 0.25)
#com.race.white <- surv.race.white.sig[com.list.race.white, ]
com.race.white <- merge(com.race.white, unlist(surv.race.white.r2), by="row.names", all.x=TRUE)
row.names(com.race.white) <- com.race.white$Row.names
com.race.white$Row.names <- NULL
colnames(com.race.white)[ncol(com.race.white)] <- "r2"
com.race.white$p2 <- pnorm(abs(com.race.white$beta/com.race.white$se), lower.tail=F)*2
com.race.white <- com.race.white[order(com.race.white$beta),]

com.race.black <- subset(surv.race.black.sig[com.list.race.black, ], abs(beta) > 0.25)
#com.race.black <- surv.race.black.sig[com.list.race.black, ]
com.race.black <- merge(com.race.black, unlist(surv.race.black.r2), by="row.names", all.x=TRUE)
row.names(com.race.black) <- com.race.black$Row.names
com.race.black$Row.names <- NULL
colnames(com.race.black)[ncol(com.race.black)] <- "r2"
com.race.black$p2 <- pnorm(abs(com.race.black$beta/com.race.black$se), lower.tail=F)*2
com.race.black <- com.race.black[order(com.race.black$beta),]
...
com.age.above.only <- subset(com.age.above, var %in% com.age.above$var[!(com.age.above$var %in% com.age.below$var)])
com.age.below.only <- subset(com.age.below, var %in% com.age.below$var[!(com.age.below$var %in% com.age.above$var)])

com.race.latin.only <- subset(com.race.latin, var %in% com.race.latin$var[ !(com.race.latin$var %in% com.race.white$var) & !(com.race.latin$var %in% com.race.black$var) ])
com.race.white.only <- subset(com.race.white, var %in% com.race.white$var[ !(com.race.white$var %in% com.race.latin$var) & !(com.race.white$var %in% com.race.black$var) ])
com.race.black.only <- subset(com.race.black, var %in% com.race.black$var[ !(com.race.black$var %in% com.race.latin$var) & !(com.race.black$var %in% com.race.white$var) ])
```
...
baseformula <- addToBase(baseformula, rownames(com.all))
