...
library(metafor, quietly=T)    # meta-analysis package
```
...
pheatmap(FinalVariableList.heat, fontsize_row=5, fontsize_col=5, annotation_row=annotation, annotation_col=annotation, annotation_colors=annotation_colors)
```
...
for (i in 1:nrow(surv.all.meta.sig)) { # number of covariates in model
  ...
  for (j in 1:(3*nrow(surv.all.meta.sig))) { # sample the model with 'i' covariates 30 times (same as total)
...
```{r}
barplot(unlist(lapply(nagelkerke, function(x) { sum(!is.na(x)) })), main="Viable Models per Number of Covariates", ylab="Viable Models (out of 30 Samplings)", xlab="Number of Covariates")
```
