...
heart <- c("MCQ160B", "MCQ160C", "MCQ160E")
cancer <- c("MCQ220")

#MainTable$chronic <- apply(MainTable, 1, FUN=function(x) { ifelse(any(x[chronic_conditions] == 1, na.rm=T), 1, 0) } )
MainTable$chronic <- 0 
MainTable$heart <- 0 
MainTable$cancer <- 0 
...
for (i in 1:nrow(MainTable)) {
  ...
  if ( any(MainTable[i, chronic_conditions] == 1, na.rm=T) ) {
    MainTable$chronic[i] <- 1
  }

  if ( any(MainTable[i, heart] == 1, na.rm=T) ) {
    MainTable$heart[i] <- 1
  }
  
  if ( any(MainTable[i, cancer] == 1, na.rm=T) ) {
    MainTable$cancer[i] <- 1
  }
}
...
variablesToKeep <- c('PERMTH_EXM', 'MORTSTAT', adjustfor, FinalVariableList, 'SDDSRVYR', 'SDMVPSU', 'SDMVSTRA', 'WTMEC2YR', 'WTMEC4YR', 'area', 'heart', 'cancer', 'UCOD')
...
MainTable.2.visits <- subset(MainTable.2.visits, num_times_health_care == 0 & times_overnight_in_hospital == 0)
...
MainTable.2.all <- subset(MainTable.2.all, num_times_health_care == 0 & times_overnight_in_hospital == 0) # remove health care users
MainTable.2.all <- subset(MainTable.2.all, chronic == 0) # remove chronic conditions
MainTable.2 <- MainTable.2.all

#MainTable.2 <- subset(MainTable.2.base, heart == 1)
#MainTable.2 <- subset(MainTable.2.base, cancer == 1)

# case 5 do 
#MainTable.2 <- subset(MainTable.2.base, UCOD == "001")
#MainTable.2 <- subset(MainTable.2.base, UCOD == "002")
#MainTable.2 <- subset(MainTable.2.base, UCOD == "003")
```
...
xwas2df <- function(surv) {
  ...
  #tmp <- cbind(tmp, q=p.adjust(tmp$p, method='fdr')) # don't do FDR until after meta-analysis
...
baseline.mod <- coxph(Surv(PERMTH_EXM, MORTSTAT) ~ 
                        ...
                        factor(RIDRETH1, c(3, 1, 2, 4, 5)) + chronic + cluster(area), 
                        weights=MainTable.2$WTINT2YR, MainTable)
...
#View(surv.all.meta.sig)
#View(surv.m.meta.sig[(row.names(surv.m.meta.sig) %in% row.names(surv.all.meta.sig)), ]) # men
#View(surv.f.meta.sig[(row.names(surv.f.meta.sig) %in% row.names(surv.all.meta.sig)), ]) # women
#View(surv.age.above.meta.sig[(row.names(surv.age.above.meta.sig) %in% row.names(surv.all.meta.sig)), ]) # old
#View(surv.age.below.meta.sig[(row.names(surv.age.below.meta.sig) %in% row.names(surv.all.meta.sig)), ]) # young
#View(surv.pir.above.meta.sig[(row.names(surv.pir.above.meta.sig) %in% row.names(surv.all.meta.sig)), ]) # rich
#View(surv.pir.below.meta.sig[(row.names(surv.pir.below.meta.sig) %in% row.names(surv.all.meta.sig)), ]) # poor
#View(surv.race.white.meta.sig[(row.names(surv.race.white.meta.sig) %in% c(row.names(surv.all.meta.sig), row.names(surv.all.meta.sig))), ]) # white
#View(surv.race.black.meta.sig[(row.names(surv.race.black.meta.sig) %in% c(row.names(surv.all.meta.sig), row.names(surv.all.meta.sig))), ]) # black
#View(surv.race.latin.meta.sig[(row.names(surv.race.latin.meta.sig) %in% c(row.names(surv.all.meta.sig), row.names(surv.all.meta.sig))), ]) # latin
```

Plot the hazard ratios versus sub-strata HRs.
...
```{r}
# sex sub-strata
sub.m <- merge(surv.all.meta.sig[, c(1,2)], surv.m.meta[, c(1,2)], by="row.names", all.x=T)
sub.f <- merge(surv.all.meta.sig[, c(1,2)], surv.f.meta[, c(1,2)], by="row.names", all.x=T)
plot(sub.m$Effect.x, sub.m$Effect.y, pch=20, xlim=c(0,2), ylim=c(0,2), frame.plot=F, col=cc[1],
                                          xlab="Unified HR", ylab="Sub-strata HR (Sex)")
points(sub.f$Effect.x, sub.f$Effect.y, pch=20, col=cc[2])
abline(0, 1, col="gray60", lty=2)
legend("topleft", bty="n", pch=20, col=cc,
       legend=c(as.expression(bquote(paste("Male ", rho, "=", .(round(cor(sub.m$Effect.x, sub.m$Effect.y, method="spearman"), digits=2))))),
                as.expression(bquote(paste("Female ", rho, "=", .(round(cor(sub.f$Effect.x, sub.f$Effect.y, method="spearman"), digits=2))))))
       )
```

```{r}
# age sub-strata
sub.age.a <- merge(surv.all.meta.sig[, c(1,2)], surv.age.above.meta[, c(1,2)], by="row.names", all.x=T)
sub.age.b <- merge(surv.all.meta.sig[, c(1,2)], surv.age.below.meta[, c(1,2)], by="row.names", all.x=T)
plot(sub.age.a$Effect.x, sub.age.a$Effect.y, pch=20, xlim=c(0,2), ylim=c(0,2), frame.plot=F, col=cc[1],
                                          xlab="Unified HR", ylab="Sub-strata HR (Age)")
points(sub.age.b$Effect.x, sub.age.b$Effect.y, pch=20, col=cc[2])
abline(0, 1, col="gray60", lty=2)
legend("topleft", bty="n", pch=20, col=cc,
       legend=c(as.expression(bquote(paste("65+ ", rho, "=", .(round(cor(sub.age.a$Effect.x, sub.age.a$Effect.y, method="spearman"), digits=2))))),
                as.expression(bquote(paste("<65 ", rho, "=", .(round(cor(sub.age.b$Effect.x, sub.age.b$Effect.y, method="spearman"), digits=2))))))
       )
```

```{r}
# pir sub-strata
sub.pir.a <- merge(surv.all.meta.sig[, c(1,2)], surv.pir.above.meta[, c(1,2)], by="row.names", all.x=T)
sub.pir.b <- merge(surv.all.meta.sig[, c(1,2)], surv.pir.below.meta[, c(1,2)], by="row.names", all.x=T)
plot(sub.pir.a$Effect.x, sub.pir.a$Effect.y, pch=20, xlim=c(0,2), ylim=c(0,2), frame.plot=F, col=cc[1],
                                          xlab="Unified HR", ylab="Sub-strata HR (Poverty:Income, PIR)")
points(sub.pir.b$Effect.x, sub.pir.b$Effect.y, pch=20, col=cc[2])
abline(0, 1, col="gray60", lty=2)
legend("topleft", bty="n", pch=20, col=cc,
       legend=c(as.expression(bquote(paste("PIR > 1 ", rho, "=", .(round(cor(sub.pir.a$Effect.x, sub.pir.a$Effect.y, method="spearman", use="complete.obs"), digits=2))))),
                as.expression(bquote(paste("PIR < 1 ", rho, "=", .(round(cor(sub.pir.b$Effect.x, sub.pir.b$Effect.y, method="spearman"), digits=2))))))
       )
```

```{r}
# race sub-strata
sub.race.a <- merge(surv.all.meta.sig[, c(1,2)], surv.race.white.meta[, c(1,2)], by="row.names", all.x=T)
sub.race.b <- merge(surv.all.meta.sig[, c(1,2)], surv.race.black.meta[, c(1,2)], by="row.names", all.x=T)
sub.race.c <- merge(surv.all.meta.sig[, c(1,2)], surv.race.latin.meta[, c(1,2)], by="row.names", all.x=T)
plot(sub.race.a$Effect.x, sub.race.a$Effect.y, pch=20, xlim=c(0,2), ylim=c(0,2), frame.plot=F, col=cc[1],
                                          xlab="Unified HR", ylab="Sub-strata HR (Race)")
points(sub.race.b$Effect.x, sub.race.b$Effect.y, pch=20, col=cc[2])
points(sub.race.c$Effect.x, sub.race.c$Effect.y, pch=20, col=cc[3])
abline(0, 1, col="gray60", lty=2)
legend("topleft", bty="n", pch=20, col=cc,
       legend=c(as.expression(bquote(paste("White ", rho, "=", .(round(cor(sub.race.a$Effect.x, sub.race.a$Effect.y, method="spearman"), digits=2))))),
                as.expression(bquote(paste("Black ", rho, "=", .(round(cor(sub.race.b$Effect.x, sub.race.b$Effect.y, method="spearman"), digits=2))))),
                as.expression(bquote(paste("Mexican-American ", rho, "=", .(round(cor(sub.race.c$Effect.x, sub.race.c$Effect.y, method="spearman"), digits=2))))))
       )
```
...
abline(0, 1, col="gray60", lty=2)
text(0.5, 1.5, bquote(paste(rho, "=", .(round(cor(df.outliers$base, df.outliers$outliers, method="spearman"), digits=2)) ))) 
...
abline(0, 1, col="gray60", lty=2)
text(0.5, 1.5, bquote(paste(rho, "=", .(round(cor(df.chronic$base, df.chronic$chronic, method="spearman"), digits=2)) ))) 
...
abline(0, 1, col="gray60", lty=2)
text(0.5, 1.5, bquote(paste(rho, "=", .(round(cor(df.visits$base, df.visits$visits, method="spearman"), digits=2)) ))) 
...
abline(0, 1, col="gray60", lty=2)
text(2.5, 1.5, bquote(paste(rho, "=", .(round(cor(df.all$base, df.all$all, method="spearman"), digits=2)) ))) 

### combined
plot(all ~ base, data=df.all, xlim=c(0,3), ylim=c(0,3), xlab="Base HR", ylab="Metric HR", main="Substratified Health Indicators", frame.plot=F, pch=20, col=rgb(0,0,0, .1))
abline(0, 1, col="gray60", lty=2)
points(visits ~ base, data=df.visits, pch=20, col=rgb(.2,.8,.2, .1))
points(chronic ~ base, data=df.chronic, pch=20, col=rgb(1,.65,0, .1))
points(outliers ~ base, data=df.outliers, pch=20, col=rgb(0,0,1, .1))
...
Heatmap for only the 45 significant measures for mortality.
...
Test significant list for r^2 values for all comorbidities.
