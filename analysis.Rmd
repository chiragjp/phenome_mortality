...
#load('~/src/xwas/survival/nhanes_schema_merged_all_031816.Rdata')
...
useTheseSeries <- c('1999-2000', '2001-2002', '2003-2004', '2005-2006', '2007-2008', '2009-2010', '2011-2012') # patient cohort series we're using
...
Do a quick table to see the variable distribution, there are about 1574 unique ones. We subset the list based on prefix using what's defined in `beginWith` and store that in `VarDescription.3`. This brings down the list of variables to about 457.

```{r}
head( table(VarDescription.2$var, VarDescription.2$series) ) # Variables by cohort year,
dim( dim(VarDescription.2$var, VarDescription.2$series) )    # 1,574 unique variables.

beginWith <- c('LBX', 'LBD', 'URX', 'URD', 'DXX', 'MGX', 'MSD', 'MSX', 'SPX', 'DEX', 'MSYS', 'MDIS', 'BIX', 'BAX', 'CVD', 'BPX', 'BMX') 
...
dim( table(VarDescription.3$var, VarDescription.3$series) )  # 457 unique variables.
```

Choose variables that are present >= 3 surveys. Since some variables appear more than once in a given survey year we map the counts > 1 to just 1 to indicate presence then sum across the rows to find unique counts across survey years. The counts >= 3 are the variables of interest and we display the original table subsetted as a sanity check. Our final list of variables present in >= 3 surveys is 336.
...
length(FinalVariableList) # final list of variables is 336
```
...
for (i in SIduplicates) {
  ...
  if (root %in% FinalVariableList) { # check for duplicate non-SI measurement 
    ...
    #FinalVariableList <- FinalVariableList[FinalVariableList != i]
...
FinalVariableList <- FinalVariableList[!(FinalVariableList %in% SIduplicates.removed)] # remove the imposters

length(FinalVariableList) # final variable list is now 309
```
...
```{r}
table(MainTable.cleaned$SDDSRVYR) # individuals by cohort year

sum(table(MainTable.cleaned$SDDSRVYR)) # total individuals over all years

# average age and standard deviation by cohort year
for (year in names(table(MainTable.cleaned$SDDSRVYR))) {
  cat(
    paste0(year, "\t",
           round(mean(subset(MainTable.cleaned, SDDSRVYR == year)$RIDAGEYR), digits=1), " ", 
           "(", round(sd(subset(MainTable.cleaned, SDDSRVYR == year)$RIDAGEYR), digits=1), ")"
           ), 
    "\n")
}
...
for (year in names(table(MainTable.cleaned$SDDSRVYR))) {
  cat(
    paste0(year, "\t",
           round(table(subset(MainTable.cleaned, SDDSRVYR == year)$RIDRETH1), digits=1), " ", 
           "(", round(prop.table(table(subset(MainTable.cleaned, SDDSRVYR == year)$RIDRETH1))*100, digits=2), ")"
           ), 
    "\n")
}
```
...
Show that demographic variables captured in `adjustfor` are significant.
...
Create UpSetR visualizations for co-morbidities across time cohorts.

```{r}
# create space of all co-morbidities found across everything ever
...
# create a list of booleans for presence of each time cohort in the union list of all co-morbidities
...
upset(com.df.all, sets=c("1999-2010", rev(AvailableSeries)), keep.order=T, order.by="freq", sets.x.label="Significant Co-morbidities \n (q < 0.05)", mainbar.y.label="Unique Co-morbidities",
...
upset(com.df.m,   sets=c("1999-2010", rev(AvailableSeries)), keep.order=T, order.by="freq", sets.x.label="Significant Co-morbidities \n (q < 0.05)", mainbar.y.label="Unique Co-morbidities",
      #set.metadata=list(data=usr.lbl.all, plots=list(list(type="text", column="CoM", assign=10))),
    queries=list(
                 list(query=intersects, params=list("1999-2010", "1999-2000", "2001-2002", "2003-2004", "2005-2006", "2007-2008", "2009-2010"), color="red", active=T),
                 list(query=intersects, params=list("1999-2010", "2001-2002", "2003-2004", "2005-2006", "2007-2008", "2009-2010"), color="red", active=T),
...
upset(com.df.f,   sets=c("1999-2010", rev(AvailableSeries)), keep.order=T, order.by="freq", sets.x.label="Significant Co-morbidities \n (q < 0.05)", mainbar.y.label="Unique Co-morbidities",
...
Re-process UpSetR data to get list of co-morbidities for all and by gender. Criteria is anything that appears in analysis across all cohorts (1999-2010) or in at least 3 different cohorts individually. Use those comorbities if they have a beta value for mortality of > 0.25 or < -0.25.
...
Now test your significant list for r^2 values.
...
***RE-THINK SIMPLIFYING THINGS BELOW THIS LINE***

Factors decreasing and increasing mortality in all people.

```{r}
cat("There are", nrow(surv.all.sig), "factors impacting mortality in humans.")

# factors decreasing mortality
head(surv.all.sig) # all over all time cohorts
for (i in 1:length(AvailableSeries)) { 
  print(cat("Series (all)", AvailableSeries[i], "\n"))
  print(head( eval(parse(text=paste0("surv.all.sig.", i))) )) 
}
...
for (i in 1:length(AvailableSeries)) { 
  print(cat("Series (all)", AvailableSeries[i], "\n"))
  print(tail( eval(parse(text=paste0("surv.all.sig.", i))) )) 
}
```

Factors decreasing and increasing mortality in just males.

```{r}
cat("There are", nrow(surv.m.sig), "factors impacting mortality for males.")

# factors decreasing mortality
head(surv.m.sig) # male over all time cohorts
for (i in 1:length(AvailableSeries)) { 
  print(cat("Series (m)", AvailableSeries[i], "\n"))
  print(head( eval(parse(text=paste0("surv.m.sig.", i))) )) 
}
...
for (i in 1:length(AvailableSeries)) { 
  print(cat("Series (all)", AvailableSeries[i], "\n"))
  print(tail( eval(parse(text=paste0("surv.m.sig.", i))) )) 
}
```

Factors decreasing and increasing mortality in just females.

```{r}
cat("There are", nrow(surv.f.sig), "factors impacting mortality for females.")

# factors decreasing mortality
head(surv.f.sig) # female over all time cohorts
for (i in 1:length(AvailableSeries)) { 
  print(cat("Series (f)", AvailableSeries[i], "\n"))
  print(head( eval(parse(text=paste0("surv.f.sig.", i))) )) 
}
...
for (i in 1:length(AvailableSeries)) { 
  print(cat("Series (all)", AvailableSeries[i], "\n"))
  print(tail( eval(parse(text=paste0("surv.f.sig.", i))) )) 
}
```

Do some visualization (should have loaded `venneuler`).
...
Co-morbidities present in both genders analyzed separately (M and F).

```{r}
unique(subset(VarDescription, var %in% names(MnF[MnF]))[, c("var", "var_desc", "var_desc_ewas")])
```

Co-morbidities present in across All and M groups.

```{r}
unique(subset(VarDescription, var %in% names(AnM[AnM]))[, c("var", "var_desc", "var_desc_ewas")])
```

Co-morbidities present in across All and F groups.

```{r}
unique(subset(VarDescription, var %in% names(AnF[AnF]))[, c("var", "var_desc", "var_desc_ewas")])
```

Co-morbidities present in across All and M and F groups.

```{r}
unique(subset(VarDescription, var %in% names(AnMnF[AnMnF]))[, c("var", "var_desc", "var_desc_ewas")])
```

Co-morbidities present only in All.

```{r}
subset(surv.all.sig, var %in% OnlyA)
```

Co-morbidities present only in M.

```{r}
subset(surv.m.sig, var %in% OnlyM)
```

Co-morbidities present only in F.

```{r}
subset(surv.f.sig, var %in% OnlyF)
```

IGNORE BELOW THIS LINE FOR NOW, storing rough draft code.

```{r eval=FALSE}
par(mfrow=c(1,1), xpd=TRUE) # setup to plot outside of area if necessary

groups <- subset(VarDescription, var_desc_ewas %in% useTheseCategories)[, c("var_desc_ewas", "var")]
groups <- unique(groups)

# http://stackoverflow.com/questions/3932038/plot-a-legend-outside-of-the-plotting-area-in-base-graphics
n <- length(unique(groups$var_desc_ewas))

margins <- par()$mar
manhattan(test, group.by=groups, group.col=rainbow(n), legend=F, xlab="", nlabels=5)
par(oma=c(4, 1, 1, 1))
par(fig=c(0, 1, 0, 1), oma=c(0, 0, 0, 0), mar=c(0, 0, 0, 0), new = TRUE)
plot(0, 0, type = "n", bty = "n", xaxt = "n", yaxt = "n")
legend("bottom", names(table(groups$var_desc_ewas)), xpd=TRUE, inset=c(0, 0), bty="n", text.col=rainbow(n), ncol=4, cex=0.7)
par(mar=margins)

top <- xwas.to.df(test) # convert xwas results to a data.frame
...
top.q <- top[order(top$q, decreasing=F), ]
top.x <- top[order(top$x, decreasing=T), ]

# show the top 10 results with p-values and descriptions
head(top, n=10)

volcano(test, xlim=c(0,2), xlab="Association Size")

glupredictrange <- c(100)
agepredictrange <- c(50)

s <- predict(glucose.m.1999, se=TRUE, type="curve", newdata=data.frame(LBXGLU=log(glupredictrange), RIDAGEYR=agepredictrange, RIDAGEYR2=agepredictrange^2, RIDRETH1=c(1)))
