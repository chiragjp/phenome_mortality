...
library(rmeta)      # for meta-analysis functions
```
...
length(FinalVariableList) # final variable list is now 256
```
...
# age above analysis - OLD
 dsn.age.above <- subset(dsn.all, RIDAGEYR >= 62)
...
# age below analysis - YOUNG
 dsn.age.below <- subset(dsn.all, INDFMPIR < 1)
...
xmeta <- function(x) {
  ...
  meta.var <- rownames(meta.beta)[apply(meta.beta, 1, function(x) { table(complete.cases(x))["TRUE"] }) > 2] # list of lab values that are significant in at least 3 cohorts

  for (i in meta.var) {
    use <- complete.cases(t(meta.beta[i, ]))
    meta.all[[i]] <- meta.summaries(d=as.numeric(meta.beta[i, use]), se=as.numeric(meta.se[i, use]), names=AvailableSeries[use], method="random")
  }
...
xmeta2df <- function(x) {
  ...
  for (i in 1:length(x)) {
    meta.summary <- summary(x[[i]])
    # overall effect, lower CI 95%, upper CI 95%, p-value, tau2 
    meta.final <- rbind(meta.final, c(meta.summary$summci[2], meta.summary$summci[1], meta.summary$summci[3], meta.summary$het[3], meta.summary$tau2))
  }
  rownames(meta.final) <- names(x)
  colnames(meta.final) <- c("Effect", "Lower", "Upper", "p", "tau2")
  ...
  meta.final <- meta.final[order(meta.final$Effect), ]
