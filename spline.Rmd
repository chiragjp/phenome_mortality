...
addToBase <- function(base_formula, adjustingVariables) {
  for (var in adjustingVariables) {
    if (var == "RIDAGEYR2") {
      base_formula <- update.formula( base_formula, as.formula('~ . + I(scale(RIDAGEYR)^2)') )
    } else if (var == "RIAGENDR" || var == "RIDRETH1" || var == "chronic") {
      base_formula <- update.formula( base_formula, as.formula(sprintf('~ . + %s', var)) )
    } else {
      base_formula <- update.formula( base_formula, as.formula(sprintf('~ . + I(scale(%s))', var)) )
    }
  }
...
xwas <- function(design=NULL, linear=FALSE, depvar="MORTSTAT", adjfor=NULL) {
  ...
  for (varname in FinalVariableList) { # probably need to parameterize FinalVariableList
    ...
    if (linear) {
      baseformula <- as.formula(sprintf('Surv(PERMTH_EXM, MORTSTAT) ~ I(%s) + cluster(area)', varname))
    } else {
      # https://cran.r-project.org/web/packages/survival/vignettes/splines.pdf
      baseformula <- as.formula(sprintf('Surv(PERMTH_EXM, MORTSTAT) ~ pspline(%s, df=4, degree=3) + cluster(area)', varname))
    }
    baseformula <- addToBase(baseformula, adjfor)
    #print("DEBUG")
    #print(baseformula)
...
```{r}
dsn.all <- subset(MainTable.cleaned, WTMEC2YR > 0)
print("*** LINEAR ***")
surv.all.raw.linear <- xwas(design=dsn.all, linear=TRUE, adjfor=adjustfor) 
print("\n")
print("*** SPLINE ***")
surv.all.raw.spline <- xwas(design=dsn.all, linear=FALSE, adjfor=adjustfor)
```

With linear and spline modeling overall, compare the fit of fits using AIC. Non-linear fit is clearly the winner for modeling.
...
nrow(l_vs_s) # 115 variables in common for models

median(l_vs_s$delta, na.rm=TRUE) # of the variables the median delta is 649116.5 >>> 10
...
Dump out all the plots of the spline overall.
...
```{r}
# to hold all the annual spline xwas results
dsn.all.spline <- list()
dsn.m.spline <- list()
dsn.f.spline <- list()
dsn.pir.above.spline <- list()
dsn.pir.below.spline <- list()
dsn.age.above.spline <- list()
dsn.age.below.spline <- list()
dsn.race.white.spline <- list()
dsn.race.black.spline <- list()
dsn.race.mexam.spline <- list()

# to hold all the beta parameter estimates per lab test
dsn.all.est <- list()
dsn.m.est <- list()
dsn.f.est <- list()
dsn.pir.above.est <- list()
dsn.pir.below.est <- list()
dsn.age.above.est <- list()
dsn.age.below.est <- list()
dsn.race.white.est <- list()
dsn.race.black.est <- list()
dsn.race.mexam.est <- list()

# to hold all the standard errors per lab test
dsn.all.se <- list()
dsn.m.se <- list()
dsn.f.se <- list()
dsn.pir.above.se <- list()
dsn.pir.below.se <- list()
dsn.age.above.se <- list()
dsn.age.below.se <- list()
dsn.race.white.se <- list()
dsn.race.black.se <- list()
dsn.race.mexam.se <- list()
```

```{r}
# cohort analysis - looping through each study year
AvailableSeries <- names(table( MainTable.cleaned$SDDSRVYR ))
for (i in 1:length(AvailableSeries)) {
  cat("*** Now analyzing", useTheseSeries[i], "series\n")
  dsn.all <- subset(MainTable.cleaned, WTMEC2YR > 0 & SDDSRVYR == AvailableSeries[i])
  
  cat("*** Subsetting cohorts in", useTheseSeries[i], "\n")
  ...
  dsn.race.white <- subset(dsn.all, RIDRETH1 == "white") # race analysis - WHITE
  dsn.race.black <- subset(dsn.all, RIDRETH1 == "black") # race analysis - BLACK
  dsn.race.mexam <- subset(dsn.all, RIDRETH1 == "mexam") # race analysis - LATIN (only mexican-american)
  
  cat("*** XWAS cohort in", useTheseSeries[i], ".\n")
  dsn.all.spline[[i]] <- xwas(design=dsn.all, linear=FALSE, adjfor=adjustfor)

  
  for (j in names(dsn.all.spline[[i]])) {
    cat("Calculating HR for", j, "...")
    tmp <- as.data.frame(dsn.all[, j])
    rownames(tmp) <- rownames(dsn.all)
    
    if (!is.null(dsn.all.spline[[i]][[j]])) {
      cat(" OK!\n")
      predicted <- predict(dsn.all.spline[[i]][[j]], x=dsn.all, type="terms", se.fit=T, terms=1)
      tmp <- merge(tmp, cbind(predicted$fit, predicted$se.fit), by="row.names")
      tmp <- tmp[, c(2,3,4)]
      tmp <- tmp[complete.cases(tmp), ]
      tmp <- tmp[order(tmp[, 1]), ]
      ...
      ## get parameter estimates (and their standard error)
      z <- seq(-2, 2, by=1)
      dsn.all.est[[j]] <- rbind(dsn.all.est[[j]], t(predict( sm.spline(tmp$var, tmp$beta), z)))
      rownames(dsn.all.est[[j]])[nrow(dsn.all.est[[j]])] <- i
  
      dsn.all.se[[j]] <- rbind(dsn.all.se[[j]], t(predict( sm.spline(tmp$var, tmp$se), z)))
      rownames(dsn.all.se[[j]])[nrow(dsn.all.se[[j]])] <- i
    } else {
      cat(" NULL\n")
    }
  }
...
```{r}
# to hold all the beta values per lab test
dsn.all.meta.beta <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.m.meta.beta <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.f.meta.beta <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.pir.above.meta.beta <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.pir.below.meta.beta <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.age.above.meta.beta <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.age.below.meta.beta <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.race.white.meta.beta <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.race.black.meta.beta <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.race.mexam.meta.beta <- data.frame(matrix(0, nrow=0, ncol=5))

# to hold all the standard errors per lab test
dsn.all.meta.se <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.m.meta.se <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.f.meta.se <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.pir.above.meta.se <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.pir.below.meta.se <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.age.above.meta.se <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.age.below.meta.se <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.race.white.meta.se <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.race.black.meta.se <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.race.mexam.meta.se <- data.frame(matrix(0, nrow=0, ncol=5))

# to hold all the p-values per lab test
dsn.all.meta.pval <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.m.meta.pval <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.f.meta.pval <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.pir.above.meta.pval <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.pir.below.meta.pval <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.age.above.meta.pval <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.age.below.meta.pval <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.race.white.meta.pval <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.race.black.meta.pval <- data.frame(matrix(0, nrow=0, ncol=5))
dsn.race.mexam.meta.pval <- data.frame(matrix(0, nrow=0, ncol=5))
```

```{r}
for (j in names(dsn.all.est)) {
  test_beta <- rep(NA, 5)
  test_se <- rep(NA, 5)
  test_pval <- rep(NA, 5)
  
  for (k in 1:5) {
    mask <- is.finite(dsn.all.est[[j]][, k])
    if (length(dsn.all.est[[j]][, k][mask]) > 0) {
      tmp <- tryCatch( {rma(yi=dsn.all.est[[j]][, k][mask], sei=dsn.all.se[[j]][, k][mask], method="REML")}, error=function(e) {})
      
      if (!is.null(tmp)) {
        test_beta[k] <- tmp$beta
        test_se[k] <- tmp$se
        test_pval[k] <- tmp$pval
      }
    }
  }
  dsn.all.meta.beta <- rbind(dsn.all.meta.beta, test_beta)
  dsn.all.meta.se <- rbind(dsn.all.meta.se, test_se)
  dsn.all.meta.pval <- rbind(dsn.all.meta.pval, test_pval)
  
  rownames(dsn.all.meta.beta)[nrow(dsn.all.meta.beta)] <- j
  rownames(dsn.all.meta.se)[nrow(dsn.all.meta.se)] <- j
  rownames(dsn.all.meta.pval)[nrow(dsn.all.meta.pval)] <- j
}

colnames(dsn.all.meta.beta) <- paste0("Z", 1:5)
colnames(dsn.all.meta.se) <- paste0("Z", 1:5)
colnames(dsn.all.meta.pval) <- paste0("Z", 1:5)

dsn.all.meta.pval <- apply(dsn.all.meta.pval, 2, function(x) { p.adjust(x, method="BY") })
dsn.all.meta.pval <- apply(dsn.all.meta.pval, 2, function(x) { ifelse(is.finite(x) & x < 0.05, x, 1) })
```

```{r}
mask <- apply(dsn.all.meta.pval, 2, function(x) { ifelse(x < 0.05, 1, 0) })
mask.sig <- ifelse(apply(mask, 1, sum) > 2, 1, 0)

mask[which(mask.sig == 1), ]
dsn.all.meta.sig <- mask[apply(mask, 1, sum) > 2, ]
```

```{r}
for (i in rownames(dsn.all.meta.sig)) {
  png(paste0("~/src/phenome-mortality/plot/sig-all-", i, ".png"))
  
  plot(-2:2, t(exp(dsn.all.meta.beta[i, ])), 
       type="l", 
       main=paste0(VarDescription[VarDescription$var==i,][1, "tab_desc"], " (", i, ")"), 
       xlab="SD",
  ...
  for (j in 1:nrow(dsn.all.est[[i]])) {
    lines(-2:2, exp(dsn.all.est[[i]][j, ]), col=rgb(0,0,0,0.2))
  }
  
  lines(-2:2, t(exp(dsn.all.meta.beta[i, ] + dsn.all.meta.se[i, ])), col=rgb(1,0,0,0.5), lty="dashed")
  lines(-2:2, t(exp(dsn.all.meta.beta[i, ] - dsn.all.meta.se[i, ])), col=rgb(1,0,0,0.5), lty="dashed")
